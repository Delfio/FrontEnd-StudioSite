...
const schema = Yup.object().shape({
  ...
  phone_contato: Yup.string().max(11, 'Maximo de 11 números'),
  email_contato: Yup.string().email('Insira um e-mail válido'),
  responsavel: Yup.string(),
  data_evento: Yup.date().required('Favor insira a data do evento'),
});
        ...
        <div className="row">
          <div className="col s12 center">
            <div className="col s6">
              <Link to="/allEventos">Todos os eventos</Link>
            </div>
            <div className="col s6">
              <Link to={`/novoEvento/${id}/files`}>Arquivos deste evento</Link>
            </div>
          </div>
        </div>
      </Container>
